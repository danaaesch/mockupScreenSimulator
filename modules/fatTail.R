fatTailUI <- function(id) {
  ns <- NS(id)
  tagList(
    div(class = "slider-endcaps", span("Skinnier"), span("Fatter")),
    shinyWidgets::sliderTextInput(
      inputId = ns("fatTail"),
      label   = "Given the mean diffCoeff for a particular well, how fat are the tails of the distribution of the FOVs from that well?",
      choices = as.character(seq(0, 10, by = 0.1)),
      selected = "6",
      grid = FALSE
    ),
    plotOutput(ns("fatTailGraph"), height = 300)
  )
}

fatTailServer <- function(id) {
  moduleServer(id, function(input, output, session) {
    
    # Exportable reactive ensuring df > 2
    dfFromFatTail <- reactive({
      val <- suppressWarnings(as.numeric(input$fatTail))
      if (is.na(val)) val <- 6
      df_raw <- 11.1 - val
      pmax(df_raw, 2.1)  # ensure finite variance
    })
    
    output$fatTailGraph <- renderPlot({
      df <- dfFromFatTail()
      x  <- seq(-5, 5, length.out = 400)
      yt <- dt(x, df = df)
      
      plot(x, yt, type = "l", lwd = 4, col = "grey40",
           xlab = "", ylab = "Density", xaxt = 'n',
           ylim = c(0, 0.42),
           main = "Shape of density of pdf generating\nFOVs from a particular well")
      
      # small annotation for df
      mtext(sprintf("t(df = %.2f)  (variance defined: df>2)", df),
            side = 3, line = -1.2, cex = 0.9, col = "grey30")
    })
    
    # Return the reactive for other modules
    list(dfFromFatTail = dfFromFatTail)
  })
}
